...
const score: HTMLElement | null = document.querySelector('.score-value');
const finalScore: HTMLElement | null = document.querySelector('.final-score > span');
const menu: HTMLElement | null = document.querySelector('.menu-screen');
const buttonPlay: HTMLElement | null = document.querySelector('.btn-play');

let direction: string | undefined, loopId: any, result: number = 0;

let audio = new Audio('/audio.mp3');

type Vel = 50 | 100 | 150 | 200 | 250;

interface CustomCobra {
    size: number;
    speed: Vel;
}

// Define um array com propriedades definidas
type Cordenada = { x: number, y: number }[];
...
class Cobra { 
    ...
    constructor(custom: CustomCobra) {
    ...
    randomPosition(): number {
        ...
        const number: number = this.randomNumber(0, canvas.width - this.size);
        return Math.round(number / 30) * 30; // faz o numero aleatorio sempre ser um multiplo de 30
    }

    // Retorna uma cor aleatoria
    randomColor(): string {
        const red: number = this.randomNumber(0, 255);
        const green: number = this.randomNumber(0, 255);
        const blue: number = this.randomNumber(0, 255);
    ...
    drawCobra(): void {
        ...
        this.cobra.forEach((position, index) => {
            ...
            if (index === this.cobra.length - 1) ctx.fillStyle = 'white';
    ...
    moveSnake(): void {
        ...
        if (direction === 'right') this.cobra.push({ x: head.x + this.size, y: head.y });
        if (direction === 'left') this.cobra.push({ x: head.x - this.size, y: head.y });
        if (direction === 'up') this.cobra.push({ x: head.x, y: head.y - this.size });
        if (direction === 'down') this.cobra.push({ x: head.x, y: head.y + this.size });
    ...
    teclaDirection(e: string): void {
        if (e === 'ArrowUp' && direction !== 'down') direction = 'up';
        if (e === 'ArrowDown' && direction !== 'up') direction = 'down';
        if (e === 'ArrowLeft' && direction !== 'right') direction = 'left';
        if (e === 'ArrowRight' && direction !== 'left') direction = 'right';
    }
    ...
    checkEat(): void {
        ...
        if (head.x === this.food.x && head.y === this.food.y) {
            this.incrementScore();
            this.cobra.push({ ...head }); // Passando as cordenadas do proximo elemento do corpo da cobra ao comer o food

            audio.play();

            // Ao a head ter a mesma posição do food, redifini a posição e a cor do food
            let x: number = this.randomPosition();
            let y: number = this.randomPosition();
    ...
    gameOver(): void {
        ...
        if (!canvas || !finalScore || !menu) return;
    ...
    checkCollision(): void {
        ...
        const head = this.cobra[this.cobra.length - 1];
        const canvasLimit: number = canvas.width - this.size;
        const neckIndex: number = this.cobra.length - 2; // Pescoço da cobra, por ela estar grudada no pescoço ser uma exceção
        const wallCollision = head.x < 0 || head.x > canvasLimit || head.y < 0 || head.y > canvasLimit; // Verificação da parede

        const selfCollision = this.cobra.find((position, index) => { // Posição da cobra e da cabeça da cobra
